...
abstract class IQuickMatchRepository {
  ...
  Future<QuickInnings> updateInnings(QuickInnings innings);
  ...
  Future<InningsPost> loadLastPost(QuickInnings innings);
...
class SQLQuickMatchRepository implements IQuickMatchRepository {
  ...
  Future<UnmodifiableListView<QuickMatch>> loadAllMatches() async {
    final entities = await _sql.query(table: Tables.quickMatches);
  ...
  Future<QuickMatch> createMatch(QuickMatch match) async {
    final id = await _sql.insert(
        table: Tables.quickMatches, values: _repackQuickMatch(match));
    final entity = await _sql
        .query(table: Tables.quickMatches, where: "id = ?", whereArgs: [id]);
  ...
  @override
  Future<QuickInnings> updateInnings(QuickInnings innings) async {
    if (innings.id == null) {
    ...
    return loadInnings(innings.id!);
  }

  @override
  Future<QuickInnings> loadInnings(int id) async {
    final entity = await _sql
        .query(table: Tables.quickInnings, where: "id = ?", whereArgs: [id]);
  ...
  Future<InningsPost> createPost(InningsPost post) async {
    final id = await _sql.insert(
        table: Tables.posts, values: _repackInningsPost(post));

    final entity =
        await _sql.query(table: Tables.posts, where: "id = ?", whereArgs: [id]);
  ...
  @override
  Future<InningsPost> loadLastPost(QuickInnings innings) async {
  ...
  Future<UnmodifiableListView<InningsPost>> loadAllPostsOf(
    ...
    final entities = await _sql.query(
        table: Tables.posts, where: "innings_id = ?", whereArgs: [innings.id]);
  ...
  Future<BowlingScore> loadBowlingScoreOf(
      QuickInnings innings, int bowlerId) async {
    final entity = await _sql.query(
        table: Tables.bowlingScores,
        where: "innings_id = ? AND player_id = ?",
  ...
  Future<UnmodifiableListView<FallOfWicket>> loadWicketsOf(
    ...
    final fallOfWickets = entities.map((e) => FallOfWicket(
          _decipherWicket(
  ...
  Map<String, Object?> _repackQuickInnings(QuickInnings innings) => {
        ...
        "extras_penalties": innings.extras.penalties,
  ...
  BattingScore _unpackBattingScore(Map<String, Object?> map) => BattingScore(
        ...
        wicket: _decipherWicket(map["wicket_type"] as int?,
            batterId: map["player_id"] as int?,
        ...
        strikeRate: map["strike_rate"] as double,
      );
  ...
  static InningsPost _unpackInningsPost(Map<String, Object?> map) {
    ...
    return switch (entity.type) {
      ...
      3 => BatterRetire(
          ...
          scoreAt: Score(entity.runs_at, entity.wickets_at),
          comment: entity.comment,
          retired: _decipherWicket(entity.wicket_type,
              batterId: entity.batter_id,
              bowlerId: null,
              fielderId: null) as Retired,
          bowlerId: entity.bowler_id,
          nonStrikerId: entity.non_striker_id,
          batterId: entity.batter_id,
        ),
